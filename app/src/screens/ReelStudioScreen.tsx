...
export function ReelStudioScreen() {
  ...
  const handleSaveReel = async () => {
    if (!photos.length) {
      NativeAlert.alert('عکس', 'حداقل یک عکس را برای ذخیره‌سازی ریلز انتخاب کنید.');
      return;
    }
    ...
    try {
      ...
      const reelTitle = `${selectedTemplate.name}`;
      const reelStartDate = startedAt ?? new Date();

      const success = await saveReel(
        selectedTemplate.id as any,
        reelTitle,
      ...
      if (success) {
        NativeAlert.alert('موفق', 'ریلز HTML با موفقیت ذخیره شد!');
        // Reset form
        setPhotos([]);
        setActivePhotoIndex(0);
        setSoundtrackUri('');
        ...
        NativeAlert.alert('خطا', 'خطا در ذخیره‌سازی ریلز HTML.');
      }
    } catch (error) {
      console.error('handleSaveReel error', error);
      NativeAlert.alert('خطا', 'خطای غیرمنتظره‌ای رخ داد.');
    } finally {
    ...
    <ScreenContainer style={styles.screen}>
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        <Card style={styles.card}>
          ...
          <Button
            title={isSavingReel ? 'در حال ذخیره‌سازی ریلز' : 'ذخیره ریلز HTML'}
            onPress={handleSaveReel}
            loading={isSavingReel}
            fullWidth
          />
