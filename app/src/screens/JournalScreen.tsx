...
import { JournalEntry, MoodType, Attachment, JournalFormValues } from '../types/journal';
import { useMemories, useAddMemory, useDeleteMemory, useMoodStats, useUpdateMemory } from '../hooks/useMemory';
import { normalizeOptionalText, normalizeTags } from '../utils/journalMetadata';
  ...
  const handleSave = async () => {
    ...
    try {
      const formValues: JournalFormValues = {
        ...
        mood,
        weather: normalizeOptionalText(weather),
    ...
    <ScreenContainer style={styles.screen}>
      ...
      {entries.length === 0 ? (
        ...
        >
            ...
            <Card key={entry.id} style={styles.entryCard}>
              ...
              {(entry.weather || entry.location || (entry.tags && entry.tags.length > 0)) && (
                <View style={{ marginBottom: spacing.sm }}>
                  {entry.weather && (
                    <Text style={styles.entryMeta}>آب‌وهوا: {entry.weather}</Text>
                  )}
                  {entry.location && (
                    <Text style={styles.entryMeta}>مکان: {entry.location}</Text>
                  )}
                  {entry.tags && entry.tags.length > 0 && (
                    <Text style={styles.entryMeta}>برچسب‌ها: {entry.tags.join('، ')}</Text>
                  )}
                </View>
              )}
      ...
      >
        ...
        <Input
          ...
          helperText="شرایط جوی آن لحظه را اضافه کنید"
        />

        <Input
          label="مکان"
          placeholder="مثلاً پارک، خانه"
          value={location}
          onChangeText={setLocation}
          helperText="جایی که این خاطره در آن رخ داده را یادداشت کنید"
        />

        <Input
          label="برچسب‌ها"
          placeholder="سفر، خانوادگی"
          value={tagsInput}
          onChangeText={setTagsInput}
          helperText="با ویرگول جدا کنید"
        />
